...
<script>
    ...
    $(document).ready(function() {
        table.DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('customer/get_table_user_konter_mitra') ?>",
                "type": "POST"
            },
            "oLanguage": {
                "sSearch": "Pencarian : ",
    ...
    function by_id(id, type) {
        if (type == 'masukan_voucher') {
            saveData = 'masukan_voucher';
        }
        if (type == 'hapus') {
            saveData = 'hapus';
        }
        $.ajax({
            ...
            success: function(response) {
                ...
                } else if (type == 'masukan_voucher') {
                    location.replace('<?= base_url('customer/kelola_voucher_mitra_konter/') ?>' + id)
                }
            }
        })
    }
    ...
    form_tambah.on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: "<?php echo base_url(); ?>customer/tambah_konter",
            method: "POST",
    ...
    form_edit.on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: "<?php echo base_url(); ?>customer/edit_konter",
            method: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
